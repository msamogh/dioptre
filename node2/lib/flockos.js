...
events.listener.use(function (req, res, next) {
    ...
    for (var prop in req.body) {
        if (req.body.hasOwnProperty(prop)) {
            if (prop == 'userId') userId = req.body[prop];
        }
    }
    var event = req.body;
    console.log(req.body);

    for (var prop in res.locals.eventTokenPayload) {
        if (res.locals.eventTokenPayload.hasOwnProperty(prop)) {
            if (prop == 'userId') localUserId = req.body[prop];
        }
    }
...
var createMethod = function createMethod(name) {
